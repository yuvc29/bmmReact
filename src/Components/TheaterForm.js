...
const TheaterForm = () =>{
  ...
  const [cities, setCities ] = useState([{value:1, label:"Delhi"}, {value:2, label:"Noida"}, {value:3, label:"Mumbai"}] )
  ...
  function handleClick() {
    const fetchData = async()=>{
      try{
        const response = await axios.post("http://localhost:8080/theater", fields)
        const data = response.data;
