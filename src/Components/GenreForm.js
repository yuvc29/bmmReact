...
const GenreForm = ({movieId}) =>{
  ...
  const genres = [{value:1, label:"comedy"}, {value:2, label:"Horror"}, {value:3, label:"Thriller"}]

//   useEffect(() => {
//     try{
//       axios.get("https://localhost:8080/genre")
//       .then(response => response.json())
//           // 4. Setting *dogImage* to the image url that we received from the response above
//       .then(data => {genres = data.map((genre)=>{return {value:genre.genreId, label:genre.name }}) }  );
//     }
//     catch(err){
